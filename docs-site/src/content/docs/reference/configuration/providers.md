...
normally keeps affinity. By default `quota` may rebind it on its next request once the source
account's effective usage threshold is reached; with `pool.cacheAffinity` on, that rebind waits until
the bound account is exhausted or otherwise cannot serve. `codexAccountAutoSwitchThresholds` can
replace the global threshold for the source account; an absent entry inherits the global value and
`0` disables only that account's usage-driven moves. Pause, cooldown, reauthentication, and failure
handling can clear or move routing independently. An unbound request has no live account binding; this can include an existing visible
